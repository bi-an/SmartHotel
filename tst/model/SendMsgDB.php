...
class SendMsgDB {
    ...
    public function saveUserLocation($data) {       
        ...
        $sql = "UPDATE  `z_users` SET  `Latitude` =  '{$Latitude}',
            `Longitude` =  '{$Longitude}',
            `CreateTime` =  '{$CreateTime}' WHERE  `z_users`.`OpenId` ='{$FromUserName}';";
        $this->db->runSql( $sql );
        if($this->db->affectedRows() < 1){//if update fails,then insert one
            $sql="INSERT INTO `z_users` (`Id`, `Latitude`, `Longitude`, `OpenId`, `Loctime`) VALUES ".
    ...
    public function getUserLocation($data){
         $FromUserName=$this->db->escape($data->FromUserName);
         $sql = "SELECT * FROM  `z_users` where OpenId = '{$FromUserName}' order by `LocTime` desc limit 1";
