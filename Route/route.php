...
$sql = "SELECT * FROM  `z_user` where OpenId = '{$fromUserName}' order by `CreateTime` desc limit 1";
$startdata = $mysql->getLine( $sql );
$sql2 = "SELECT * FROM  `z_hotel` LIMIT 0 , 30";
...
<div class="block">
    ...
    <div class="select">
        ...
        foreach ($enddatas as $enddata) 
        {
            ...
            {
                echo "<option value='{$endPoint}' selected='selected'>{$enddata['Name']}</option>";
            }
            else
            {
                echo "<option value='{$endPoint}'>{$enddata['Name']}</option>";
            }       
        }
        ?>
        </select>
