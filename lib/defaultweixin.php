    ...
    private function goroute($data) {
        ...
        $posts = array(
            array(
                ...
                'url' => 'http://kladz.applinzi.com/zhzg/Route/route.php?user='. $data->FromUserName,
            )
        );
    ...
    private function goreserve($data) {
        ...
        if (!empty($ret) && $ret['dinertime'] > time() - 1800) {
            $posts = array(
                array(
                    'title' => '您最近的预约',
                    'discription' => '时间：' . date('Y-m-d H:i', $ret['dinertime']) . '，人数：' . $ret['num'],
                    ...
                    'url' => 'http://kladz.applinzi.com/zhzg/Order/seat/myreserve.php?user=' . $data->FromUserName,
                )
            );
        } else {
            $posts = array(
                array(
                    ...
                    'discription' => '现在预约吧',
                    'picurl' => 'http://mmbiz.qpic.cn/mmbiz/XWia2Xj7RZ8nxGcFl47qJQjsm1iaqf3SquWeL0BzAficTdkxo2oeV9PvVqvcUUj14pE4oq5fAZx2s4TGsZIalZFCg/0',
                    'url' => 'http://kladz.applinzi.com/zhzg/Order/seat/reserve.php?user=' . $data->FromUserName,
                )
            );
        }
    ...
    private function showqr($data, $type = 0) {
        ...
        $posts = array(
            array(
                ...
                'url' => 'http://kladz.applinzi.com/zhzg/showqr.php?sceneid=' . $sceneid . '&user=' . $data->FromUserName,
            )
        );
        $this->outputNews($posts);
    }
