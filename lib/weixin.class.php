...
class weixin extends wxcommon
{
  ...
  public static function getQrcodeTicket( $scene_id = 0, $expire = 0 ){
    ...
    public static function getOpenUrl($url) {
        $url = urlencode($url);
        $appid = APPID;
        return "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$appid}&redirect_uri={$url}&response_type=code&scope=snsapi_base&state=235#wechat_redirect";
    }
